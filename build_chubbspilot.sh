...
# VS Code workspace safety check
check_vscode_environment() {
    # There are several ways VS Code might identify itself
    if [[ "$TERM_PROGRAM" == "vscode" ]] || [[ -n "$VSCODE_IPC_HOOK" ]] || [[ -n "$VSCODE_CWD" ]] || [[ -n "$VSCODE_PID" ]]; then
        echo "Running in VS Code terminal - good!"

        # Check if we're in a git repo
        if git rev-parse --is-inside-work-tree > /dev/null 2>&1; then
            echo "Git repository detected - continuing..."
            return 0
        else
            echo "ERROR: Not in a git repository. Please run this script from within your workspace git repository."
            return 1
        fi
    else
        echo "ERROR: This script must be run from within a VS Code terminal."
        return 1
    fi
}
...
if ! check_vscode_environment; then
    echo "SAFETY CHECK FAILED: This script is designed to run only within a VS Code workspace."
    echo "Running it elsewhere could result in accidental file deletion on your system."
    exit 1
fi
